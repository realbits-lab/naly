...
class EnhancedPPTGenerator:
    ...
    def get_shape_type_enum(self, shape_type_str: str) -> Optional[MSO_SHAPE]:
        ...
        import re
        clean_name = re.sub(r'\s*\(\d+\)(\s*\(\d+\))*', '', shape_type_str).strip().upper()
        ...
        try:
            ...
            if 'SOLID' in fill_type or fill_info.get('solid'):
                ...
                if 'back_color' in fill_info:
                    self.apply_enhanced_color(shape.fill.back_color, fill_info['back_color'])
    ...
    def apply_enhanced_line(self, shape, line_info: Dict[str, Any]):
        ...
        try:
            ...
            print(f"Warning: Could not apply enhanced line properties: {str(e)}")
                    ...
                    for paragraph in text_frame.paragraphs:
                        for run in paragraph.runs:
                            ...
                            if row_idx == 0:
                                ...
                                if hasattr(self, 'theme_fonts'):
                                    font_info = self.theme_fonts.get('major_font', {})
                                    if font_info.get('latin'):
                                        run.font.name = font_info['latin']
                            else:
                                run.font.size = Pt(10)
        ...
        if text:
            ...
            for run in p.runs:
                ...
                if shape_info.get('fill', {}).get('fore_color'):
                    self.apply_color_to_font(run.font, shape_info['fill']['fore_color'])
    ...
    def get_optimal_layout(self, shapes: List[Dict[str, Any]]) -> Any:
        ...
        chart_count = sum(1 for s in shapes if 'CHART' in s.get('shape_type', ''))
        table_count = sum(1 for s in shapes if 'TABLE' in s.get('shape_type', ''))
    ...
    def generate_enhanced_slides(self):
        ...
        for slide_data in self.shapes_data:
            ...
            shapes_sorted = sorted(shapes, key=lambda x: x.get('shape_index', 0))
    ...
    def save_presentation(self, output_file: str):
        ...
        print(f"\nEnhanced presentation saved to: {output_file}")
        print(f"Total slides: {len(self.presentation.slides)}")


def main():
    parser = argparse.ArgumentParser(
        description='Generate enhanced PowerPoint presentation from JSON files with improved fidelity'
    )
    ...
    try:
        ...
        generator.generate_enhanced_slides()
        
        # Save presentation
        generator.save_presentation(args.output)
        
        print(f"\nEnhanced presentation generation completed successfully!")
        print(f"Fidelity improvements applied:")
